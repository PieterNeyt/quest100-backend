...
func CreateAgendaRequestBody(employeeId int, isStudent bool) *RequestBody {
	...
	now := time.Now()
	startOfDay := time.Date(now.Year(), now.Month(), now.Day()-1, 0, 0, 0, 0, now.Location()).Unix()
	endOfDay := time.Date(now.Year(), now.Month(), now.Day()-1, 23, 59, 59, 0, now.Location()).Unix()
